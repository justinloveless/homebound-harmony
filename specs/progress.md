...
| 10 | Dockerfile + Coolify | **Done** — combined `Dockerfile` (Node builds the SPA, Bun runs the server with `./public` as the SPA) + `server/Dockerfile` (server-only). `docker-compose.yml` runs three services: `postgres`, `app` (production-style API+SPA on `:3000`), and `web` (Vite dev SPA on `:8080` with HMR; proxies `/api`, `/healthz`, and `/s/:id/data` to `app`). `.env.example` documents required secrets. One-shot bring-up: `cp .env.example .env && docker compose up --build`. `.dockerignore` keeps `server/drizzle/` (migrations run on boot). Verified locally: postgres goes healthy, app runs migrations, `GET /` returns the SPA, `GET /healthz` returns `ok`, `GET /api/auth/me` returns 401. |
